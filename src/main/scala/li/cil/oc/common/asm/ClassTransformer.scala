...
class ClassTransformer extends IClassTransformer {
    ...
    try {
      if (!name.startsWith("net.minecraft.")
        ...
        if (name.startsWith("li.cil.oc.")) {
          ...
          val missingInterfaces = classNode.interfaces.asScala.filter(!classExists(_))
          for (interfaceName <- missingInterfaces) {
            log.trace(s"Stripping interface $interfaceName from class $name because it is missing.")
          }
          classNode.interfaces.removeAll(missingInterfaces.asJavaCollection)
          val missingClasses = classNode.innerClasses.asScala.filter(clazz => clazz.outerName != null && !classExists(clazz.outerName))
          ...
          classNode.innerClasses.removeAll(missingClasses.asJavaCollection)

          val incompleteMethods = classNode.methods.asScala.filter(method => missingFromSignature(method.desc).nonEmpty)
          ...
          classNode.methods.removeAll(incompleteMethods.asJavaCollection)

          // Inject available interfaces where requested.
          if (classNode.visibleAnnotations != null) {
            def injectInterface(annotation: AnnotationNode): Unit = {
              val values = annotation.values.asScala.grouped(2).map(buffer => buffer.head -> buffer.last).toMap
                    ...
                    case Some(mod) =>
                      if (mod.isModAvailable) {
                        ...
                        if (node == null) {
                          ...
                          val missing = node.methods.asScala.filterNot(im => classNode.methods.asScala.exists(cm => im.name == cm.name && im.desc == cm.desc)).map(method => s"Missing implementation of ${method.name + method.desc}")
                          if (missing.isEmpty) {
            ...
            classNode.visibleAnnotations.asScala.find(_.desc == "Lli/cil/oc/common/asm/Injectable$Interface;") match {
              case Some(annotation) =>
                injectInterface(annotation)
              case _ =>
            }
            classNode.visibleAnnotations.asScala.find(_.desc == "Lli/cil/oc/common/asm/Injectable$InterfaceList;") match {
              case Some(annotation) =>
                val values = annotation.values.asScala.grouped(2).map(buffer => buffer.head -> buffer.last).toMap
                values.get("value") match {
                  case Some(interfaceList: java.lang.Iterable[AnnotationNode]@unchecked) =>
                    interfaceList.asScala.foreach(injectInterface)
                  case _ =>
                }
              case _ =>
            }
        ...
        }
        {
          ...
          if (classNode.interfaces.contains("li/cil/oc/api/network/SimpleComponent") &&
            (classNode.visibleAnnotations == null || !classNode.visibleAnnotations.asScala.
      ...
      if (ObfNames.Class_EntityLiving.contains(name.replace('.', '/'))) {
        ...
        insertInto(classNode, ObfNames.Method_recreateLeash, ObfNames.Method_recreateLeashDesc, instructions => instructions.toArray.sliding(3, 1).exists {
          case Array(varNode: VarInsnNode, fieldNode: FieldInsnNode, jumpNode: JumpInsnNode)
            ...
            classNode.fields.asScala.find(field => ObfNames.Field_leashedToEntity.contains(field.name)) match {
              case Some(field) =>
  ...
  def injectEnvironmentImplementation(classNode: ClassNode): Array[Byte] = {
    ...
    def replace(methodName: String, methodNameSrg: String, desc: String):Unit = {
      ...
      if (classNode.methods.asScala.exists(method => method.name == methodName + SimpleComponentImpl.PostFix && mapper.mapMethodDesc(method.desc) == desc)) {
        throw new InjectionFailedException(s"Delegator method name '${methodName + SimpleComponentImpl.PostFix}' is already in use.")
      }
      classNode.methods.asScala.find(filter) match {
        case Some(method) =>
        ...
        case _ =>
          ...
          @tailrec def ensureNonFinalIn(name: String):Unit = {
            if (name != null) {
              ...
              if (node != null) {
                node.methods.asScala.find(filter) match {
                  case Some(method) =>
          ...
          template.methods.asScala.find(_.name == methodName + SimpleComponentImpl.PostFix) match {
            case Some(method) => classNode.methods.add(method)
            case _ => throw new AssertionError(s"Couldn't find '${methodName + SimpleComponentImpl.PostFix}' in template implementation.")
          }
      }
      template.methods.asScala.find(filter) match {
        case Some(method) => classNode.methods.add(method)
